...
function onTrigger(player,npc)
    local FellowQuest = player:getCharVar("[Quest]Unlisted_Qualities")
		if player:getQuestStatus(JEUNO,tpz.quest.id.jeuno.UNLISTED_QUALITIES) == QUEST_ACCEPTED and player:getMaskBit(FellowQuest,"[Quest]Unlisted_Qualities",0) == false then
