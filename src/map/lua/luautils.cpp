...
#define lua_prepscript(n,...) int8 File[255]; \
    ...
    ************************************************************************/
    ...
    /************************************************************************
    *  onMobFamilyInitialize                                                *
    *  Used for family behavior event hookup                                *
    *                                                                       *
    ************************************************************************/
    ...
    int32 OnMobFamilyInitialize(CMobEntity* PMob)
    {
        ...
        if (!PMob->GetLocalVar("SkipOnMobFamilyInitialize") == 0)
        {
            return 0;
        }

        lua_prepscript("scripts/mobfamilies/%s.lua", PMob->GetFamilyName());

        if (prepFile(File, "onMobFamilyInitialize"))
        {
            return -1;
        }
        ...
        if (lua_pcall(LuaHandle, 1, 0, 0))
        {
            ShowError("luautils::onMobFamilyInitialize: %s\n", lua_tostring(LuaHandle, -1));
            lua_pop(LuaHandle, 1);
            return -1;
        }
    ...
    int32 ApplyInstanceMixins(CBaseEntity* PMob)
    {
        ...
        if (PMob->objtype == TYPE_MOB)
        {
            lua_prepscript("scripts/mixins/instances/%s.lua", PMob->PInstance->GetName());
...
bool OnChocoboDig(CCharEntity* PChar, bool pre)
    {
		...
		// fetch the zone digging cap value
		int32 value = 0;

		int32 ret = Sql_Query(SqlHandle, "SELECT value FROM server_variables WHERE name = '[DIG]ZONE%s_ITEMS' LIMIT 1;", PChar->getZone());
		...
		if (canDig == false) // not allowed to dig
		{
			if (PChar->getStorage(LOC_INVENTORY)->SearchItem(4545) == ERROR_SLOTID ) // player does not have gysahl greens with them
			{
				PChar->pushPacket(new CMessageSystemPacket(4545, 0, 39));
			}
			else if (charutils::GetCharVar(PChar, "[DIG]DigCount") >= ((100 + (10 * PChar->RealSkills.rank[59])))) // player digging is capped for the day
			{
				PChar->pushPacket(new CChatMessagePacket(PChar, MESSAGE_SYSTEM_3,"You have maxed your player digging for today!"));
			}
			else if (value >= 120) // the zone is capped for the vana hour
			{					
				PChar->pushPacket(new CChatMessagePacket(PChar, MESSAGE_SYSTEM_3,"This zone has nothing left to dig for!"));
			}
			else	// one of the timers is not met
			{
		...
		{
		((CCharEntity*)PChar)->loc.zone->PushPacket(PChar, CHAR_INRANGE_SELF, new CChocoboDiggingPacket(PChar)); // play dig animation
		}
    return 0;
    }
    ...
    int PushMobList(lua_State* L, std::vector<fishmob_t>* MobList) {
        ...
        if (MobList != nullptr) {
            for (std::vector<fishmob_t>::iterator mob = MobList->begin(); mob != MobList->end(); ++mob) {
                ...
                lua_settable(L, -3);
                lua_pushstring(L, "minLength");
                lua_pushnumber(L, mob->minLength);
                lua_settable(L, -3);
                lua_pushstring(L, "maxLength");
                lua_pushnumber(L, mob->maxLength);
                lua_settable(L, -3);
                lua_pushstring(L, "size");
                lua_pushnumber(L, mob->difficulty);
                lua_settable(L, -3);
                lua_pushstring(L, "baseDelay");
                ...
                lua_settable(L, -3);
                lua_pushstring(L, "questOnly");
                lua_pushboolean(L, mob->questOnly);
                lua_settable(L, -3);
                lua_pushstring(L, "nm");
                ...
                lua_settable(L, -3);
                lua_pushstring(L, "maxRespawn");
                lua_pushnumber(L, mob->maxRespawn);
                lua_settable(L, -3);
                lua_pushstring(L, "reqKeyItem");
                lua_pushnumber(L, mob->reqKeyItem);
                lua_settable(L, -3);
                lua_pushstring(L, "reqBait");
                lua_pushnumber(L, mob->reqBaitId);
                lua_settable(L, -3);
                lua_pushstring(L, "altBait");
                lua_pushnumber(L, mob->altBaitId);
                lua_settable(L, -3);
                lua_pushstring(L, "zoneId");
                lua_pushnumber(L, mob->zoneId);
                lua_settable(L, -3);
