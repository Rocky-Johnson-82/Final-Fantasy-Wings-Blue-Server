...
void SmallPacket0x00A(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    if (PChar->status == STATUS_DISAPPEAR)
    {
        ...
        {
            Sql_Query(SqlHandle, "UPDATE flist_settings SET lastonline = %u WHERE callingchar = %u;", (uint32)CVanaTime::getInstance()->getVanaTime(),
                      PChar->id);
        }
    ...
    PChar->PAI->QueueAction(queueAction_t(1000ms, false, luautils::AfterZoneInLong));
}
...
void SmallPacket0x015(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    {
        float new_x = data.ref<float>(0x04);
        float new_y = data.ref<float>(0x08);
        float new_z = data.ref<float>(0x0C);

        bool moved = ((PChar->loc.p.x != new_x) || (PChar->loc.p.z != new_z) || (PChar->m_TargID != data.ref<uint16>(0x16)));
        ...
        if (!PChar->isCharmed)
        {
            ...
            PChar->loc.p.movingsnapshot = PChar->loc.p.moving;
...
void SmallPacket0x01A(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    switch (action)
    {
        ...
        case 0x11: // chocobo digging
        {
			...
			if (PChar->isMounted())
			{
			luautils::OnChocoboDig(PChar, true);
			}
        }
        break;
        case 0x12: // dismount
            ...
            case 0x02: // trade accepted
            {
                ...
                if (PChar->TradePending.id == PTarget->id && PTarget->TradePending.id == PChar->id)
                {
                    ...
                    if (PTarget->UContainer->IsLocked())
                    {
                        ...
                        {
                            uint32 TradeID = charutils::DoTrade(PChar, PTarget);
                            PTarget->pushPacket(new CTradeActionPacket(PTarget, 9));

                            charutils::DoTrade(PTarget, PChar, TradeID);
...
void SmallPacket0x04B(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    uint32 ret = Sql_Query(SqlHandle, "SELECT FLsentnotif FROM accounts_sessions WHERE charid = %u LIMIT 1;", PChar->id);
    ...
    if (ret != SQL_ERROR && Sql_NumRows(SqlHandle) == 1 && Sql_NextRow(SqlHandle) == SQL_SUCCESS)
    {
        if (Sql_GetUIntData(SqlHandle, 0) == 0) // we haven't sent notifs yet, this is the first server message request since their account session started
        {
            ...
            {
                ShowError("SQL error occurred in packet_system.cpp under Server Message Request to UPDATE accounts_sessions SET FLsetnotif = 1 WHERE charid = "
                          "%u. Exiting function to prevent multiple notifications upon each character /servmes\n",
                          PChar->id);
                return;
            }

            FLnotify(PChar, false, false); // send notifications to friends

            if (FLgetSetting(PChar, 2) == 0) // invisible and just logged in
            {
                int32 channel = FLgetSetting(PChar, 4);
                PChar->pushPacket(new CChatMessagePacket(PChar, (CHAT_MESSAGE_TYPE)channel, "== FLIST MESSAGE ==", ""));
                PChar->pushPacket(new CChatMessagePacket(PChar, (CHAT_MESSAGE_TYPE)channel, "You have logged in with your hidden status on.", ""));
                PChar->pushPacket(new CChatMessagePacket(PChar, (CHAT_MESSAGE_TYPE)channel, "Login notifications were not sent to your friends.", ""));
                PChar->pushPacket(
                    new CChatMessagePacket(PChar, (CHAT_MESSAGE_TYPE)channel, "Use '!flist unhide' or '!flist unhide silent' to disable this setting.", ""));
            }
        }
        return;
    }
    ...
    {
        ShowError("SQL error occurred in packet_system.cpp under Server Message Request to SELECT FLsentnotif FROM accounts_sessions WHERE charid = %u ... did "
                  "you remember to ALTER accounts_sessions to add in this new column?\n",
                  PChar->id);
    }
...
void SmallPacket0x04D(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    if ((PChar->animation >= ANIMATION_FISHING_FISH && PChar->animation <= ANIMATION_FISHING_STOP) ||
        PChar->animation == ANIMATION_FISHING_START_OLD || PChar->animation == ANIMATION_NEW_FISHING_START ||
        PChar->animation == ANIMATION_NEW_FISHING_FISH || PChar->animation == ANIMATION_NEW_FISHING_CAUGHT ||
        PChar->animation == ANIMATION_NEW_FISHING_ROD_BREAK || PChar->animation == ANIMATION_NEW_FISHING_LINE_BREAK ||
        PChar->animation == ANIMATION_NEW_FISHING_MONSTER || PChar->animation == ANIMATION_NEW_FISHING_STOP)
    {
    ...
    switch (action)
    {
        ...
        case 0x07: // remove received items from send box
        {
            ...
            int32 ret = Sql_Query(SqlHandle, "SELECT slot, itemid, itemsubid, quantity, senderid, sender FROM delivery_box WHERE charid = %u AND received = 1 AND box = 2 ORDER BY slot ASC;", PChar->id);
            ...
            if (ret != SQL_ERROR)
            {
                ...
                if (received_items && Sql_NextRow(SqlHandle) == SQL_SUCCESS)
                {
                    ...
                    itemID = Sql_GetUIntData(SqlHandle, 1);
                    itemSubID = Sql_GetUIntData(SqlHandle, 2);
                    quantity = Sql_GetUIntData(SqlHandle, 3);
                    senderID = Sql_GetUIntData(SqlHandle, 4);
                    sender = (char*)Sql_GetData(SqlHandle, 5);
                    if (!PChar->UContainer->IsSlotEmpty(slotID))
                    {
                        CItem* PItem = PChar->UContainer->GetItem(slotID);
                        if (PItem->isSent())
                        {
                            Sql_Query(SqlHandle, "INSERT INTO dbox_log (charid, charname, box, itemid, itemsubid, quantity, senderid, sender, op) VALUES (%u, '%s', 2, %u, %u, %u, %u, '%s', 1);", PChar->id, PChar->name.c_str(), itemID, itemSubID, quantity, senderID, sender.c_str());
                    ...
                    if (ret != SQL_ERROR && Sql_NumRows(SqlHandle) > 0 && Sql_NextRow(SqlHandle) == SQL_SUCCESS)
                    {
                        ...
                        if (senderID != 0)
                        {
                            ...
                            if (ret != SQL_ERROR && Sql_AffectedRows(SqlHandle) > 0)
                            {
                                ...
                                Sql_Query(SqlHandle, "INSERT INTO dbox_log (charid, charname, box, itemid, itemsubid, quantity, senderid, sender, op) VALUES (%u, '%s', 1, %u, %u, %u, %u, '%s', 2);", PChar->id, PChar->name.c_str(), PItem->getID(), PItem->getSubID(), PItem->getQuantity(), senderID, senderName.c_str());
        ...
        case 0x0A: // Option: Take
        {
            ...
            if (!PChar->UContainer->IsSlotEmpty(slotID))
            {
                ...
                CItem* PItem = PChar->UContainer->GetItem(slotID);
                int32 ret = SQL_ERROR;
                ret = Sql_Query(SqlHandle, "SELECT itemid, itemsubid, quantity, senderid, sender FROM delivery_box WHERE charid = %u AND slot = %u AND box = %u LIMIT 1", PChar->id, slotID, boxtype);
                if (ret == SQL_ERROR || Sql_NextRow(SqlHandle) != SQL_SUCCESS) {
                    return;
                }
                itemID = Sql_GetUIntData(SqlHandle, 0);
                itemSubID = Sql_GetUIntData(SqlHandle, 1);
                quantity = Sql_GetUIntData(SqlHandle, 2);
                senderID = Sql_GetUIntData(SqlHandle, 3);
                sender = (char*)Sql_GetData(SqlHandle, 4);
                ...
                if (Sql_SetAutoCommit(SqlHandle, false) && Sql_TransactionStart(SqlHandle))
                {
                    if (boxtype == 0x01)
                    {
                        Sql_Query(SqlHandle, "INSERT INTO dbox_log (charid, charname, box, itemid, itemsubid, quantity, senderid, sender, op) VALUES (%u, '%s', 1, %u, %u, %u, %u, '%s', 3);", PChar->id, PChar->name.c_str(), itemID, itemSubID, quantity, senderID, sender.c_str());
                        ret = Sql_Query(SqlHandle, "DELETE FROM delivery_box WHERE charid = %u AND slot = %u AND box = %u LIMIT 1", PChar->id, slotID, boxtype);
                    }
                    else if (boxtype == 0x02)
                    {
                        Sql_Query(SqlHandle, "INSERT INTO dbox_log (charid, charname, box, itemid, itemsubid, quantity, senderid, sender, op) VALUES (%u, '%s', 2, %u, %u, %u, %u, '%s', 3);", PChar->id, PChar->name.c_str(), itemID, itemSubID, quantity, senderID, sender.c_str());
        ...
        case 0x0B: // Option: Drop
        {
            ...
            if (!PChar->UContainer->IsSlotEmpty(slotID))
            {
                ...
                int32 ret = SQL_ERROR;
                ret = Sql_Query(SqlHandle, "SELECT itemid, itemsubid, quantity, senderid, sender FROM delivery_box WHERE charid = %u AND slot = %u AND box = 1 LIMIT 1", PChar->id, slotID);
                if (ret == SQL_ERROR || Sql_NextRow(SqlHandle) != SQL_SUCCESS) {
                    return;
                }
                itemID = Sql_GetUIntData(SqlHandle, 0);
                ...
                Sql_Query(SqlHandle, "INSERT INTO dbox_log (charid, charname, box, itemid, itemsubid, quantity, senderid, sender, op) VALUES (%u, '%s', 1, %u, %u, %u, %u, '%s', 4);", PChar->id, PChar->name.c_str(), itemID, itemSubID, quantity, senderID, sender.c_str());
                ret = Sql_Query(SqlHandle, "DELETE FROM delivery_box WHERE charid = %u AND slot = %u AND box = 1 LIMIT 1", PChar->id, slotID);
...
void SmallPacket0x066(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    if ((FISHACTION)action != FISHACTION_FINISH || anim == ANIMATION_FISHING_FISH)
    {
        // ShowDebug("0x066 if is true, doing FishingAction...\n");
...
void SmallPacket0x074(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    if (PChar->nameflags.flags & FLAG_INVITE && InviteAnswer)
    {
...
void SmallPacket0x0B5(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    CChatMessagePacket* NewMessage = nullptr;
    ...
    {
        NewMessage = new CChatMessagePacket(PChar, MESSAGE_SYSTEM_1, (const char*)data[9]);
        if (map_config.audit_chat && map_config.audit_system) {
            NewMessage->LogChat(nullptr, nullptr);
        }
        message::send(MSG_CHAT_SERVMES, 0, 0, NewMessage);
    }
    else if (data.ref<uint8>(0x06) == '#' && PChar->m_GMlevel > 0)
    {
        if (PChar->m_GMlevel > 1) {
            NewMessage = new CChatMessagePacket(PChar, MESSAGE_SYSTEM_1, (const char*)data[7]);
            if (map_config.audit_chat && map_config.audit_system) {
                NewMessage->LogChat(nullptr, nullptr);
            }
            message::send(MSG_CHAT_SERVMES, 0, 0, NewMessage);
    ...
    else
    {
        if (jailutils::InPrison(PChar))
        {
            ...
            {
                NewMessage = new CChatMessagePacket(PChar, MESSAGE_SAY, (const char*)data[6 + msg_offset]);
                if (map_config.audit_chat == 1 && ((map_config.audit_say == 1) || (PChar->m_logChat)))
                {
                    NewMessage->LogChat(PChar->name.c_str(), nullptr);
                }
                PChar->loc.zone->PushPacket(PChar, CHAR_INRANGE, NewMessage);
            }
            else
            {
                PChar->pushPacket(new CMessageBasicPacket(PChar, PChar, 0, 0, 316));
        ...
        else
        {
            switch (data.ref<uint8>(0x04))
            {
                ...
                case MESSAGE_EMOTION:
                    NewMessage = new CChatMessagePacket(PChar, MESSAGE_EMOTION, (const char*)data[6 + msg_offset]);
                    if (map_config.audit_chat == 1 && ((map_config.audit_emotion == 1) || (PChar->m_logChat)))
                    {
                        NewMessage->LogChat(nullptr, nullptr);
                    }
                    PChar->loc.zone->PushPacket(PChar, CHAR_INRANGE, NewMessage);
                ...
                case MESSAGE_SHOUT:
                {
                    NewMessage = new CChatMessagePacket(PChar, MESSAGE_SHOUT, (const char*)data[6 + msg_offset]);
                    if (map_config.audit_chat == 1 && ((map_config.audit_shout == 1) || (PChar->m_logChat)))
                    {
                        NewMessage->LogChat(nullptr, nullptr);
                    }
                    PChar->loc.zone->PushPacket(PChar, CHAR_INSHOUT, NewMessage);
                }
                break;
                case MESSAGE_LINKSHELL:
                {
                    if (PChar->PLinkshell1 != nullptr)
                    {
                        NewMessage = new CChatMessagePacket(PChar, MESSAGE_LINKSHELL, (const char*)data[6 + msg_offset]);
                        int8 packetData[8]{};
                        ...
                        {
                            int8 ls1name[24] = { 0 };
                            DecodeStringLinkshell(PChar->PLinkshell1->getName(), ls1name);
                            NewMessage->LogChat(nullptr, (char*)ls1name);
                        }
                        message::send(MSG_CHAT_LINKSHELL, packetData, sizeof packetData, NewMessage);
                    }
                }
                break;
                case MESSAGE_LINKSHELL2:
                {
                    if (PChar->PLinkshell2 != nullptr)
                    {
                        ...
                        if (map_config.audit_chat == 1 && ((map_config.audit_linkshell == 1) || (PChar->m_logChat) || (PChar->PLinkshell2->getAuditSettings())))
                        {
                            ...
                            NewMessage->LogChat(nullptr, (char*)ls2name);
                        }
                        message::send(MSG_CHAT_LINKSHELL, packetData, sizeof packetData, NewMessage);
                    }
                }
                ...
                case MESSAGE_PARTY:
                {
                    if (PChar->PParty != nullptr)
                    {
                        NewMessage = new CChatMessagePacket(PChar, MESSAGE_PARTY, (const char*)data[6 + msg_offset]);
                        int8 packetData[8]{};
                        ref<uint32>(packetData, 0) = PChar->PParty->GetPartyID();
                        ref<uint32>(packetData, 4) = PChar->id;
                        if (map_config.audit_chat == 1 && ((map_config.audit_party == 1) || (PChar->m_logChat)))
                        {
                            NewMessage->LogChat(nullptr, nullptr);
                        }
                        message::send(MSG_CHAT_PARTY, packetData, sizeof packetData, NewMessage);
                    }
                }
                break;
                case MESSAGE_YELL:
                {
                    ...
                    if (PChar->loc.zone->CanUseMisc(MISC_YELL) && YellCheck == EYellCheckResult::YELLDEC_SUCCESS)
                    {
                        if (gettick() >= charutils::GetCharUVar(PChar, "NextYell"))
                        {
                            ...
                            NewMessage = new CChatMessagePacket(PChar, MESSAGE_YELL, (const char*)data[6 + msg_offset]);
                            if (map_config.audit_chat == 1 && ((map_config.audit_yell == 1) || (PChar->m_logChat)))
                            {
                                NewMessage->LogChat(nullptr, nullptr);
                            }
                            message::send(MSG_CHAT_YELL, packetData, sizeof packetData, NewMessage);
                        }
...
void SmallPacket0x0B6(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    uint8 recipientNameOffset = 5;
    uint8 messageContentOffset = 20;
    if (PChar->m_needTellFix)
    {
        recipientNameOffset = 6;
    ...
    message::send(MSG_CHAT_TELL, packetData, RecipientName.length() + 5, NewMessage);
}
    ...
    switch (data.ref<uint8>(0x04))
    {
        ...
        case 3: // change merit
        {
            if (PChar->m_moghouseID == PChar->id)
            {
                ...
                {
                    bool tookEffect = true;
                    switch (operation)
                    {
                        case 0:
                            tookEffect = PChar->PMeritPoints->LowerMerit(merit);
                            break;
                        case 1:
                            tookEffect = PChar->PMeritPoints->RaiseMerit(PChar, merit);
                            break;
                    }
                    if (tookEffect)
                    {
    ...
    if (type == 0x02)
    {
        if (PChar->id == id)
        {
            ...
            if (PChar->getEquip(SLOT_RANGED) && PChar->getEquip(SLOT_RANGED)->isType(ITEM_WEAPON))
            {
                ...
                PChar->pushPacket(
                    new CMessageBasicPacket(PChar, PChar, PChar->RACC(skill, bonusSkill), PChar->RATT(skill, 999.9f, bonusSkill), MSGBASIC_CHECKPARAM_RANGE));
            }
        ...
        else if (PChar->PPet && PChar->PPet->id == id)
        {
            ...
            if (((CPetEntity*)(PChar->PPet))->getPetType() == PETTYPE_AUTOMATON)
            {
                int skill = SKILL_AUTOMATON_RANGED;
            ...
            else if (PChar->getEquip(SLOT_RANGED) && PChar->getEquip(SLOT_RANGED)->isType(ITEM_WEAPON))
            {
                int skill = ((CItemWeapon*)PChar->getEquip(SLOT_RANGED))->getSkillType();
                PChar->pushPacket(new CMessageBasicPacket(PChar, PChar->PPet, PChar->PPet->RACC(skill), PChar->PPet->RATT(skill), MSGBASIC_CHECKPARAM_RANGE));
            }
...
void SmallPacket0x0E7(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    else if (PChar->animation == ANIMATION_NONE)
    {
        ...
        if (PChar->PPet == nullptr || (PChar->PPet->m_EcoSystem != SYSTEM_AVATAR && PChar->PPet->m_EcoSystem != SYSTEM_ELEMENTAL))
        {
...
void SmallPacket0x0E8(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    switch (PChar->animation)
    {
        case ANIMATION_NONE:
        {
            ...
            if (PChar->PPet == nullptr ||
                (PChar->PPet->m_EcoSystem != SYSTEM_AVATAR && PChar->PPet->m_EcoSystem != SYSTEM_ELEMENTAL && !PChar->PAI->IsEngaged()) && !PChar->m_moghouseID)
            {
...
void SmallPacket0x0F1(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    uint16 EffectID = IconID;

    if (IconID == 444 || IconID == 445 || IconID == 446) // utsusemi can have different icons
        EffectID = 66;

    if (IconID)
    {
        CStatusEffect* effect = PChar->StatusEffectContainer->GetStatusEffect((EFFECT)EffectID);

        if (effect == nullptr)
            return;

        // think this covers all the removable effects
        if (effect->GetFlag() & EFFECTFLAG_CONFRONTATION || effect->GetFlag() & EFFECTFLAG_FOOD || effect->GetFlag() & EFFECTFLAG_ERASABLE)
            return;
...
void SmallPacket0x106(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    if ((PBazaarItem->getCharPrice() != 0) && (PBazaarItem->getQuantity() >= Quantity))
    {
        ...
        char buyer_name[16] = { 0 };
        char seller_name[16] = { 0 };
        Sql_EscapeStringLen(SqlHandle, buyer_name, PChar->name.c_str(), sizeof(buyer_name) < PChar->name.length() ? sizeof(buyer_name) : PChar->name.length());
        Sql_EscapeStringLen(SqlHandle, seller_name, PTarget->name.c_str(), sizeof(seller_name) < PTarget->name.length() ? sizeof(seller_name) : PTarget->name.length());
        const char* Query = "INSERT INTO bazaar_log (seller_id, seller_name, buyer_id, buyer_name, item_id, single_price, quantity, total_sale, tax, total_paid) VALUES (%u, '%s', %u, '%s', %u, %u, %u, %u, %u, %u);";
        Sql_Query(SqlHandle, Query, PTarget->id, seller_name, PChar->id, buyer_name, PItem->getID(), PBazaarItem->getCharPrice(), Quantity, Price, PriceWithTax-Price, PriceWithTax);
...
void SmallPacket0x110(map_session_data_t* const PSession, CCharEntity* const PChar, CBasicPacket data)
{
    ...
    if (anim != ANIMATION_NEW_FISHING_FISH && anim != ANIMATION_NEW_FISHING_CAUGHT && anim != ANIMATION_NEW_FISHING_ROD_BREAK &&
        anim != ANIMATION_NEW_FISHING_LINE_BREAK && anim != ANIMATION_NEW_FISHING_MONSTER && anim != ANIMATION_NEW_FISHING_STOP &&
        anim != ANIMATION_NEW_FISHING_START)
        return;
    // ShowDebug("SmallPacket0x110 into FishingAction...\n");
    // uint32 charid = data.ref<uint32>(0x04);
    uint16 stamina = data.ref<uint16>(0x08);
    // uint16 ukn1 = data.ref<uint16>(0x0A); // Seems to always be zero with basic fishing, skill not high enough to test legendary fish.
    // uint16 targetid = data.ref<uint16>(0x0C);
