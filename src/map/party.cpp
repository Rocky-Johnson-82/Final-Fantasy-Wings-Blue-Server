...
void CParty::SetSyncTarget(int8* MemberName, uint16 message)
{
    ...
    if (map_config.level_sync_enable)
    {
        if (PEntity && PEntity->objtype == TYPE_PC)
        {
            ...
            for (uint8 i = 0; i < members.size(); ++i)
            {
                if (members.at(i)->GetMLevel() > PEntity->GetMLevel() + 20)
                {
                    ((CCharEntity*)GetLeader())
                        ->pushPacket(new CChatMessagePacket((CCharEntity*)GetLeader(), MESSAGE_SYSTEM_1,
                                                            "Party members must be within a 20 level range of the sync target.", "Server"));
                    return;
                }
            }
